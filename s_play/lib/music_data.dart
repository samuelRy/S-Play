...
(String, String, String, String, int, int, Uint8List) metaC = ("", "", "", "", -1, -1, Uint8List(0));

ValueNotifier<(int, String, Uint8List)> currentMusic = ValueNotifier<(int, String, Uint8List)>(current);
ValueNotifier<(String, String, String, String, int, int, Uint8List)> currentMeta = ValueNotifier<(String, String, String, String, int, int, Uint8List)>(metaC);
ValueNotifier<Uint8List?> backU8List = ValueNotifier<Uint8List?>(backImageDefault);

List<String> folders = ["C:/Users/elija/Desktop/S-Play/Sounds Tests"];
...
class PlayList{
  PlayList({required this.id, required this.name, this.image, this.audios = const {}});
  final int id;
  String name;
  Image? image;
...
ValueNotifier<bool> playListInitializedNotifier = ValueNotifier<bool>(playListInitialized);
Future<void> initializePlayList() async {
  // print("object");
  playListInitializedNotifier.value = false;
  List<String> sounds = [];
  for (var folder in folders) {
    await for(FileSystemEntity entity in Directory(folder).list(recursive : true, followLinks : false)){
...
ValueNotifier<Map<dynamic, List<dynamic>>> mappedNotifier = ValueNotifier<Map<dynamic, List<dynamic>>>(mappedList);